...
const SPREAD = 340; /* px between card centers on the arc */
/* Scroll px per card. With 6 projects this gives smooth transitions */
          ...
          {PROJECTS.map((p, i) => (
            <article className={styles.card} key={p.slug} style={{ zIndex: 100 - i }}>
              <div className={styles.inner}>
                <Link className={styles.cardHeaderLink} href={`/work/${p.slug}`}>
                  <div
                ...
                <div className={styles.metaFoot}>
                  ...
                  <div className={styles.footActions}>
                    <Link href={`/work/${p.slug}`} className={styles.open}>
                      {t("work.open")} <i>→</i>
                    </Link>
                    <a
                      className={styles.liveBtn}
                      href={p.site?.url || p.repo || "#"}
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`${t("work.live")} - ${p.title}`}
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        const url = p.site?.url || p.repo;
              ...
              {/* GitHub repository chip over cover's top-right */}
              {p.repo && (
                <a
