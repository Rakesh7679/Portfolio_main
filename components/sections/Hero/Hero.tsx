...
const STATS_LEFT = [
  { n: 6, suffix: "+", key: "stat.projects", icon: "/images/icons/projects.png" },
  { n: 2, suffix: "+", key: "stat.years", icon: "/images/icons/years.png" },
];
const STATS_RIGHT = [
  { n: 12, suffix: "+", key: "stat.countries", icon: "/images/icons/countries.png" },
  { n: 10, suffix: "+", key: "stat.satisfaction", icon: "/images/icons/satisfaction.png" },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    mm.add("(max-width: 1000px)", () => {
      /* Mobile: set stat numbers directly and clear inline GSAP styles */
      gsap.utils.toArray<HTMLElement>(`.${styles.statNum}`).forEach((numEl) => {
        const target = Number(numEl.dataset.count || 0);
        if (numEl.firstChild) {
          numEl.firstChild.textContent = String(target);
        }
      });
      ...
      targets.forEach((sel) => {
        gsap.utils.toArray<HTMLElement>(sel).forEach((node) => {
          gsap.set(node, {
            clearProps: "transform,opacity,visibility,x,y,z,scale,autoAlpha",
          });
        });
      });
    });
      ...
      <div className={styles.stageRow}>
        ...
        <div className={styles.stage}>
          ...
          <Image
            ...
            alt="Rakesh Laha — Fullstack Developer"
            width={500}
            height={500}
            priority
            data-depth="0.03"
